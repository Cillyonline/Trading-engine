...
1. `src/api/main.py` mounts `/ui` and includes the bounded API routers used by the shared runtime shell.
2. `src/ui/index.html` contains the runtime shell and browser workflow markers for analysis, watchlists, alerts, journal inspection, and runtime state.
3. `src/api/test_operator_workbench_surface.py` verifies `/ui` reachability and linked runtime markers.
4. `tests/test_ui_runtime_browser_flow.py` verifies the shared-shell browser workflow against the existing runtime API surface.
