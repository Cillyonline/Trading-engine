...
This document separates what the API guarantees from what it explicitly does not guarantee. It is limited to the documented API behavior in `docs/operations/api/usage_contract.md`, the operator access contract in `docs/operations/access-policy.md`, and the reserved runtime lifecycle control contract in `docs/architecture/engine_runtime_lifecycle_contract.md`.
...
- The API guarantees that the request and response shapes documented in `docs/operations/api/usage_contract.md` define the MVP v1.1 contract.
- The API guarantees that the documented field requirements, enums, and ranges in `docs/operations/api/usage_contract.md` define the MVP v1.1 contract.
- The API guarantees that `docs/operations/access-policy.md` is the authoritative role-to-endpoint contract for the current operator-facing routes in `src/api/main.py`.
...
- The API guarantees deterministic denial semantics for protected operator-facing endpoints: unauthenticated requests map to `401 Unauthorized`, and authenticated requests without the required role map to `403 Forbidden` as documented in `docs/operations/external/error_semantics.md`.
...
- The API does not guarantee compatibility with request or response shapes not documented in `docs/operations/api/usage_contract.md`.
- The API does not guarantee schema stability outside the MVP v1.1 contract documented in `docs/operations/api/usage_contract.md`.
- The API does not guarantee an operator access contract for routes that are not listed in `docs/operations/access-policy.md`.
- The API does not guarantee anonymous access to any operator-facing route covered by `docs/operations/access-policy.md`.
