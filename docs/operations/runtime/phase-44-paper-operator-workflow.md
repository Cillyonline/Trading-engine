...
## Purpose
Define the minimum bounded Phase 44 operator workflow using existing decision evidence, portfolio inspection, paper execution inspection, and reconciliation surfaces.
...
3. Inspect portfolio-aware paper account/trade/position views derived from canonical entities.
...
6. Confirm the explicit decision evidence -> portfolio inspection -> paper execution -> reconciliation reference chain.
7. Treat the result as bounded paper-runtime coherence evidence only.
...
positions, and reconciliation mismatches. These counts are inspection evidence
only; they do not establish trader validation or operational readiness.
...
5. **Periodic scheduled review** — at minimum once per trading week, execute the full bounded operator inspection path.
...
1. Run the full bounded operator inspection path.
...
| R3 | Portfolio position inspection | `GET /portfolio/positions` | `total` matches workflow `inspection_summary.portfolio_positions` |
| R4 | Canonical order count | `GET /trading-core/orders` | Readable, `total >= 0` |
| R5 | Canonical execution-event count | `GET /trading-core/execution-events` | Readable, `total >= 0` |
...
- **Weekly review artifacts**: `docker compose --env-file .env -f docker/staging/docker-compose.staging.yml exec api python /app/scripts/generate_weekly_review.py --db-path /data/db/cilly_trading.db --evidence-dir /data/artifacts/weekly-review` - produces deterministic weekly review evidence bundles.
